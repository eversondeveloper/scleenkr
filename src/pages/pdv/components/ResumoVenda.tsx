// src/pages/pdv/components/ResumoVenda.tsx
...
interface ResumoVendaProps {
  ...
  finalizarVenda: () => void;
  cancelarVenda: () => void;
  pagamentosMistos?: PagamentoMisto[]; // opcional no uso atual
  valorPagoTotal: number;
  metodoSecundario?: string; // não usado neste componente, mas passado no ComponenteVendas; pode ser ignorado
  somClick?: () => void;          // ← adicione
  somClickMenos?: () => void;    // ← adicione
}
...
const ResumoVenda: React.FC<ResumoVendaProps> = ({
  metodoPagamento,
  totalGeral,
  valorFaltando, // mantido por compatibilidade, mas usamos valorFaltandoReal internamente
  valorTroco,
  podeFinalizarVenda,
  finalizarVenda,
  cancelarVenda,
  ...
  const formatarMetodosMistos = (): string => {
    if (metodoPagamento !== 'Misto' || metodosUsadosMisto.length === 0) {
  ...
  return (
    <div className="flex flex-col h-full w-full gap-3.75 box-border">
        ...
        {metodoPagamento === 'Misto' && metodosUsadosMisto.length > 0 && (
          <div className="bg-secondary/30 rounded-xl p-4 border border-border">
            {metodosUsadosMisto.map((pagamento, index) => (
              <div
                key={index}
                className="flex justify-between py-2 border-b border-border text-xs last:border-b-0"
              >
                <span className="text-muted-foreground">{pagamento.metodo}</span>
                <strong className="text-foreground">
                  R$ {formatarParaReal(pagamento.valor)}
                </strong>
              </div>
            ))}
            <div className="mt-2.5 pt-2.5 border-t border-border flex justify-between items-center">
              <span className="text-muted-foreground font-extrabold text-[10px]">
                TOTAL RECEBIDO
              </span>
              <strong className="text-success text-lg font-black">
                R$ {formatarParaReal(valorPagoTotal)}
              </strong>
            </div>
          </div>
        )}
        ...
        <div className="mt-auto">
          {!vendaIniciada ? (
            <div className="rounded-2xl p-5 text-center bg-card border border-border">
              <div className="text-muted-foreground font-bold tracking-widest text-sm">
                AGUARDANDO ITENS...
              </div>
            </div>
          ) : temPendencia ? (
            <div className="rounded-2xl p-5 text-center bg-destructive/10 border border-destructive/30 transition-all duration-300">
              <label className="block text-[10px] font-extrabold text-destructive tracking-widest uppercase mb-1.5">
                VALOR RESTANTE
              </label>
              <div className="text-destructive text-[32px] font-black">
                R$ {formatarParaReal(valorFaltandoReal)}
              </div>
            </div>
          ) : temTroco ? (
            <div className="rounded-2xl p-5 text-center bg-background border-2 border-primary transition-all duration-300 shadow-[0_0_15px_rgba(var(--primary),0.1)]">
              ...
              <div className="text-success text-[32px] font-black">
                R$ {formatarParaReal(valorTroco)}
              </div>
            </div>
          ) : pagoIntegralmente ? (
      ...
      <div className="flex gap-3 pt-2.5 shrink-0">
        <button
          ...
          className="flex-1 h-13.75 rounded-xl border border-destructive/30 font-extrabold text-[13px] cursor-pointer transition-all uppercase bg-destructive/10 text-destructive hover:bg-destructive hover:text-destructive-foreground active:scale-95"
          title="Atalho: ESC"
        >
          CANCELAR [ESC]
        </button>
        ...
        <button
          ...
          className={`
            flex-1 h-13.75 rounded-xl border border-border font-extrabold text-[13px] transition-all uppercase
            ${podeFinalizarVenda
