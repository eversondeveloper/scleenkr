// src/pages/pdv/components/ProdutosSelecionados.tsx
...
interface ProdutosSelecionadosProps {
  ...
  handleQuantidadeChange: (idUnico: string | number, novaQuantidade: number) => void;
  totalGeral: number;
...
// Componente
// ---------------------------------------------------------
const ProdutosSelecionados: React.FC<ProdutosSelecionadosProps> = ({
  ...
  const { gerarOrcamentoPDF, gerandoOrcamento } = useGerarOrcamento() as {
    gerarOrcamentoPDF: (params: any) => Promise<void>;
    gerandoOrcamento: boolean;
  }; // TODO: tipar useGerarOrcamento quando convertê-lo

  const [mostrarModalOrcamento, setMostrarModalOrcamento] = useState<boolean>(false);
  ...
  const [valorTemp, setValorTemp] = useState<Record<string | number, string>>({});
  const somCancelaRef = useRef<HTMLAudioElement>(new Audio(somCancelar));
  ...
  const handleAjusteQuantidade = useCallback(
    (idUnico: string | number, delta: number) => {
      const produto = produtosSelecionados.find((p) => p.idUnico === idUnico);
      if (!produto) return;
      const atual = parseFloat(String(produto.quantidade)) || 0;
  ...
  const handleAjusteInteligente = useCallback(
    (idUnico: string | number, valorBotao: number) => {
      const produto = produtosSelecionados.find((p) => p.idUnico === idUnico);
      if (!produto) return;
      const quantidadeAtual = parseFloat(String(produto.quantidade)) || 0;
  ...
  return (
    <div className="w-full h-full flex flex-col bg-background border border-border rounded-2xl overflow-hidden">
      ...
      <div className="p-4 bg-card border-b border-border flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              if (window.confirm('Esvaziar carrinho?'))
                produtosSelecionados.forEach((p) => removerProduto(p.idUnico));
            }}
            title="Limpar tudo"
          >
            LIMPAR
      ...
      >
        {produtosSelecionados.length === 0 ? (
          ...
          produtosSelecionados.map((produto) => {
            const valorItem = parseFloat(String(produto.preco)) || 0;
            const totalDoItem = valorItem * produto.quantidade;
            const valorParaExibir =
              valorTemp[produto.idUnico] !== undefined
                ? valorTemp[produto.idUnico]
                : produto.quantidade.toString().replace('.', ',');

            return (
              <div
                className="bg-card border border-border rounded-xl p-3 transition-colors hover:border-muted-foreground/50"
                key={produto.idUnico}
              >
                {/* Info Topo */}
                <div className="flex justify-between mb-3">
                  <div>
                    ...
                    <h4 className="text-[13px] text-foreground m-0 mt-0.5 font-medium">
                ...
                <div className="flex justify-between items-center bg-background p-2.5 rounded-lg gap-2.5">
                  <div className="flex flex-col gap-1.5 flex-1 min-w-0">
                    ...
                    <div className="flex items-center bg-secondary/30 rounded-full border border-border p-0.5 max-w-25">
                      <button
                        type="button"
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          if (processado !== '' && !isNaN(Number(processado))) {
                    ...
                    <div className="grid grid-cols-4 gap-1">
                      {[5, 10, 50].map((val) => (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleAjusteInteligente(produto.idUnico, val);
                            somClick();
                          }}
                        >
                          {val}
                  ...
                  <div className="text-right min-w-fit pl-2">
                    <span className="text-[10px] text-muted-foreground block">
                      Un: R$ {formatarParaReal(valorItem)}
                    </span>
                    <strong className="text-sm text-success font-bold">
                      R$ {formatarParaReal(totalDoItem)}
                    </strong>
                  </div>
                </div>
              </div>
      ...
      {produtosSelecionados.length > 0 && (
        <div className="p-4 bg-card border-t border-border shrink-0">
          <div className="flex justify-between items-center mb-3">
            ...
            <strong className="text-foreground text-xl font-extrabold">
      ...
      {mostrarModalOrcamento && (
        <div
          className="fixed inset-0 bg-black/85 backdrop-blur-sm flex items-center justify-center z-3000"
          onClick={() => setMostrarModalOrcamento(false)}
        >
          <div
            className="bg-background border border-border rounded-3xl w-[90%] max-w-112.5 overflow-hidden shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-5 bg-card flex justify-between items-center border-b border-border">
              <div className="flex items-center gap-2.5">
                <span className="text-xl">📄</span>
                <h3 className="m-0 text-base text-foreground tracking-wide font-bold">
                  DADOS DO ORÇAMENTO
                </h3>
              </div>
              <button
                className="bg-transparent border-none text-muted-foreground cursor-pointer text-xl hover:text-destructive transition-colors"
                onClick={() => setMostrarModalOrcamento(false)}
              >
                ✕
              </button>
            ...
            <div className="p-6 flex flex-col gap-5">
              <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                  <label className="text-[10px] text-primary font-extrabold uppercase tracking-wide">
                    NOME DO CLIENTE
                  </label>
                  <input
                    type="text"
              ...
              <div className="p-5 bg-card rounded-2xl border border-border text-center">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <strong className="text-success text-2xl font-extrabold">
                    R$ {formatarParaReal(totalGeral)}
                  </strong>
                </div>
                <p className="text-[10px] text-muted-foreground m-0">
                  Válido por 30 dias após a emissão
                </p>
              </div>
            </div>
            ...
            <div className="p-5 bg-card flex gap-3 border-t border-border">
              <button
                className="flex-1 p-3.5 rounded-xl border-none font-extrabold text-sm cursor-pointer transition-colors bg-secondary text-foreground hover:bg-muted-foreground/20"
                onClick={() => setMostrarModalOrcamento(false)}
              >
                VOLTAR [ESC]
              </button>
              <button
                className="flex-1 p-3.5 rounded-xl border-none font-extrabold text-sm cursor-pointer transition-colors bg-primary text-primary-foreground hover:brightness-110"
