// src/pages/pdv/components/FiltrosCatalogo.tsx
...
interface FiltrosCatalogoProps {
  ...
  processarBuscaDireta?: (termo: string, limpar: () => void) => void;
}
...
const FiltrosCatalogo: React.FC<FiltrosCatalogoProps> = ({
  ...
  const containerFiltrosRef = useRef<HTMLDivElement>(null);
  ...
  const handleKeyDownBusca = (e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter' && filtroBusca.trim() !== '') {
      processarBuscaDireta?.(filtroBusca, () => setFiltroBusca(''));
    }
  };
  ...
  return (
    <div
      ...
      className={`
        ...
        ${filtrosExpandidos 
          ? 'max-h-200 border border-[#333] shadow-[0_10px_30px_rgba(0,0,0,0.5)]' 
          : 'max-h-12 border border-[#222]'
        }
      `}
    ...
    >
      {/* Cabeçalho Fixo */}
      <div className="px-4.5 flex justify-between items-center bg-[#1a1a1a] h-12 min-h-12 z-10 shrink-0">
      ...
      <div className={`
        px-4.5 flex flex-col gap-5 overflow-hidden
        transition-all duration-400 ease-[cubic-bezier(0.16,1,0.3,1)]
        ${filtrosExpandidos ? 'max-h-175 opacity-100 py-5 pointer-events-auto' : 'max-h-0 opacity-0 py-0 pointer-events-none'}
      `}>
